...
public class Bray extends Action {
	...
	public boolean perform() {
		...
		if (subject.getEnergy() - energyCost >= 0 && subject.canUseAction() && performable()) {
			...
			showEffect();
			for (Action a : target.getEffects()) {
				if (tas.isChanneling(a)) {
					target.removeEffect(a.getName());
				}
	...
	public void showEffect() {
			...
			player.sendString(
					"effecttt,create,stun.png," + name + "," + target.getName()
	...
	public String getDescription() {
		return name + "\nType: " + this.type + "\nEnergy: " + energyCost
				+ "\nTime: " + timeCost
				+ "\nEffect: Stuns target for one turn. ";
	}
	...
	public Action getNew() {
		return new Bray(null, null, null, null);
