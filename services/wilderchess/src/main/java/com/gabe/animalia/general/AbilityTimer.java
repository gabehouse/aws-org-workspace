...
// public AbilityTimer(Action action, Player player, Player otherPlayer, int
// iterator, Game game) {
// this.action = action;
// this.player = player;
...
// if (!(action.isInterrupted() || (action.getTargetType() ==
// Targetable.TYPE_FIGHTER
// && ((Critter) action.getTarget()).getSpot().getName().contains("Bench")))
// && (action.getType() == ActionCategoryEnum.ITEM
// || action.getEnergyCost() <= action.getSubject().getEnergy())) {
// if (action.getSubject() == null) {
// action.perform();
// action.animate(player, otherPlayer);
// } else if ((!action.getSubject().isBenched() || (action
// .getName().equals("Unbench")
// && action
// .isStartPerformable()))
// && !(action.getTargetType() == Targetable.TYPE_FIGHTER && !((Critter) action
// .getTarget()).isAlive())) {
...
// player.setCombinedCompletedActionCount(player.getCombinedCompletedActionCount()
// + 1);
// otherPlayer.setCombinedCompletedActionCount(otherPlayer.getCombinedCompletedActionCount()
// + 1);
...
// System.out.println("animate: " + player.getCombinedCompletedActionCount() +
...
// || (action.getTargetType().equals(Targetable.TYPE_FIGHTER && ((Critter)
// action
// .getTarget()).isBenched())) {
// startPerformable = false;
// }
