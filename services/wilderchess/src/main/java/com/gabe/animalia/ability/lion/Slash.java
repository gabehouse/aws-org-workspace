...
public class Slash extends Action {
	...
	private String description = name + "\nDamage: " + damage + "\nType: " + this.type  +  "\nEnergy: " + energyCost + "\nTime: " + timeCost + "\nEffect: Raise morale by 1.";
	private String selectDescription = name + "\nDamage: " + damage + "\nType: " + this.type  +  "\nEnergy: " + energyCost + "\nTime: " + timeCost + "\nEffect: Raise morale by 1.";

	public Slash(Critter subject, Targetable target, Player player, Player otherPlayer) {
	...
	public boolean perform() {
		if (subject.getEnergy() - energyCost >= 0 && subject.canUseAction() && performable()) {
			...
			if (target.getDefence() == 0) {
				target.onHit(player, otherPlayer, this);
			} else {
				target.onBlock(player, otherPlayer);
			}
	...
	public Action getNew() {
		return new Slash(null, null, null, null);
	}
