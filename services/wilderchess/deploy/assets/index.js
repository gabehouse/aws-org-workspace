...
function setupWebSocket() {
	...
	const host = window.location.host; // This automatically gets Wilderchess.eba-... or localhost:8080
	this.ws = new WebSocket(`${protocol}//${host}/`);
	//this.ws = new WebSocket('ws://localhost:8080/');
	//	this.ws = new WebSocket('ws://wilderchess.eba-2ftygckg.us-east-2.elasticbeanstalk.com/');
	...
	this.ws.onmessage = function (evt) {
		...
		} else if (tokens[0] == "formationsetcancel") {
			formationSetCancel(evt.data);
		}
...
var totalItemNames = ["CRIMBERRY_PACK", "WAR_HORN", "OIL_BOMB", "COXCOMB", "THIEF_GLOVES", "SMOKE_BOMB"];
var WIDTH = 1080;
var HEIGHT = 675;
...
var uiX = window.screen.availWidth / 2 - 540;
var uiY = window.screen.availHeight / 2 - 338;
var critterSelectPaper = Raphael(uiX, uiY, WIDTH, HEIGHT);
...
var battleAiButton = new Button(
	critterSelectPaper.rect(WIDTH / 2 + 5, 10, 150, 40, 6).attr({ "fill": "yellow", 'stroke-width': 2 }),
	critterSelectPaper.text(WIDTH / 2 + 80, 30, "Vs. Bot").attr("font-family", buttonFont).attr("font-size", 18),
	critterSelectPaper,
	empty
);
...
var botDropdown = {
	...
	buttons: [],
	toggle: function () {
		if (battleAiButton.isDisabled()) return;
		this.isOpen = !this.isOpen;
		this.buttons.forEach(btn => this.isOpen ? btn.show() : btn.hide());
	}
};

// 3. Attach the toggle to the main button
battleAiButton.rect.unclick().click(function () { botDropdown.toggle(); });
battleAiButton.text.unclick().click(function () { botDropdown.toggle(); });

// 4. Create the difficulty options (aligned under the main button)
var botOptions = [
	{ label: "Easy", color: "#90EE90", value: "easy" },
	{ label: "Medium", color: "#FFD700", value: "medium" },
	{ label: "Hard (Neural Net)", color: "#FF4500", value: "hard" }
];

botOptions.forEach(function (opt, index) {
	var bx = battleAiButton.rect.attr('x');
	var by = battleAiButton.rect.attr('y') + (index + 1) * 45;

	var r = botOptionsPaper.rect(bx, by, 150, 40, 6).attr({ "fill": opt.color }).hide();
	var t = botOptionsPaper.text(bx + 75, by + 20, opt.label).attr("font-family", buttonFont).hide();

	// Ensure these elements are clickable on the transparent layer
	r.node.style.pointerEvents = "auto";
	t.node.style.pointerEvents = "auto";

	var btn = new Button(r, t, botOptionsPaper, function () {
		console.log("Selected Difficulty: " + opt.value);
		ws.send("playbot," + opt.value);
		// Your logic to start the bot game goes here
		botDropdown.toggle();
	});

	btn.enable(); // Sub-buttons start enabled
	botDropdown.buttons.push(btn);
});
...
var abil1Rect = new InfoBox(leftAbilRectX, topAbilRectY, abilRectWidth, abilRectHeight, critterSelectPaper, "", 12, 8);
var abil2Rect = new InfoBox(rightAbilRectX, topAbilRectY, abilRectWidth, abilRectHeight, critterSelectPaper, "", 12, 8);
var abil3Rect = new InfoBox(leftAbilRectX, botAbilRectY, abilRectWidth, abilRectHeight, critterSelectPaper, "", 12, 8);
var abil4Rect = new InfoBox(rightAbilRectX, botAbilRectY, abilRectWidth, abilRectHeight, critterSelectPaper, "", 12, 8);
var bpRect = new InfoBox(abilRectWidth + leftAbilRectX, bAbilRectY, bAbilRectWidth, bAbilRectHeight, critterSelectPaper, "", 12, 8);
...
moraleCircles.hover(function () {
	infoBox.setInfo("Morale determines the likelihood of going first when two actions are scheduled to execute at the same time. Unblocked attacks raise morale.");
})
moraleCircles.hide();
...
function actionOptions(str) {
	console.log("ACITON OPTIONS: " + str);
	var tokens = str.split(",");
	var toShowOptions = [];
...
function updateActionLog(str) {
	console.log("updateactionlog: " + str);
	var tokens = str.split(",");
	var str = tokens[1];
	var p = document.createElement("p");
	p.innerHTML = str;
	...
	// OPTIONAL: Add a subtle border or background to "box" each entry
...
var infoBox = new InfoBox(30, 430, (WIDTH / 2 - 240) - 50, HEIGHT - 445, gridPaper, "Game Info", 25, 12.5);
infoBox.hide(true);
...
function InfoBox(x, y, width, height, paper, title, distanceFromTop, fontSize) {
	...
	this.setInfo = function (str) {
		...
		// 1. STRIP UNDERSCORES IMMEDIATELY
		// This ensures spaces are counted correctly for wrapping logic
		var cleanStr = str.replace(/_/g, " ");

		// 2. Calculate character limit
		var charsPerLine = Math.floor(this.width / (this.fontSize * 0.63));

		// 3. Wrap the clean text
		var wrappedStr = wrapText(cleanStr, charsPerLine);

		// 4. THE FIX: Double newlines
		var spacedStr = wrappedStr.replace(/\n\n/g, "\n \n");

		// 5. Render
		this.info = paper.text(this.x + 8, this.y + distanceFromTop - 7, spacedStr)
			.attr({
				"text-anchor": "start",
				"font-size": this.fontSize,
				"font-family": infoBoxTextFont
			});
...
function formationSetCancel() {
	console.log("FORMATIONSETCANCEL");
	locked = false;
	readyButton.getRect()[1].attr('text', 'Ready');
	initialPhase = true;
	formationReadied = false;

}

function acceptFormation() {
	formationReadied = true;
...
function initialPhaseEnd(str) {
	...
	var positioningStr = mainParts[0];
	var snapshotParts = mainParts.slice(1);
	...
	snapshotParts.forEach(function (entry) {
		var t = entry.split(",");
		if (t[0] !== "action") return;

		// Based on your generateSystemSnapshot order:
		// t[4] = name, t[5] = side, t[6] = currentHP, t[8] = currentEnergy, t[17] = subBundle
		var name = t[4];
		var side = t[5];
		var hp = parseInt(t[6]);
		var energy = parseInt(t[8]);
		var effectBundle = t[17];

		var critter = identifyCritter(name, side);
		if (critter) {
			critter.setHealth(hp);
			critter.setEnergy(energy);

			// Apply the Bench/Aura icons immediately
			syncEffects(critter, effectBundle);

			console.log("Synced Initial Effects for: " + name + " (" + side + ")");
		}
	});
}
...
function transition(str) {
	...
	lowerRect.animate({ y: lowerFrameY }, 750, function () {
		animate(str);
	});
}

function calculateTurn(str) {
	console.log("CALCULATE TURN");
	var tokens = str.split(",");
	if (tokens[1] == "begin") {
		timeBar();
		var i = 0;
...
function Effect(x, y, imgName, type, effect, source, duration, stacks) {
	// 1. Create a Raphael Set to group visuals for easy cleanup and unified interaction
	this.group = gridPaper.set();

	// 2. Create the Icon Image
	this.img = gridPaper.image("assets/EffectSigns/effect" + imgName, x, y - 19, 12, 12);
	this.group.push(this.img);
	...
	// 4. Handle Stacks Text (Only if > 1)
	this.stackText = null;
	if (this.stacks > 1) {
		this.stackText = gridPaper.text(x + 10, y - 11, this.stacks)
			.attr({
				"fill": "white",
				"font-size": "8px",
				"font-weight": "bold"
		...
		// Direct CSS injection to bypass Raphael/Browser hover conflicts
		var textNode = this.stackText.node;
		textNode.style.pointerEvents = "none"; // Mouse passes through to the group/image
		textNode.style.userSelect = "none";    // Prevents text-selection cursor
		textNode.style.webkitUserSelect = "none";
		textNode.style.cursor = "pointer";

		this.group.push(this.stackText);
	}

	// 5. Interaction Logic (Applied to the group so the text doesn't block the image)
	this.group.hover(
		function () {
			document.body.style.cursor = 'pointer';
		},
	...
	// 6. Description Logic (Dynamic based on stacks)
	this.updateDescription = function () {
		// Just use the pre-formatted block from the server
	...
	// 8. Cleanup Helper (Call this when clearing effects to remove all parts at once)
	this.remove = function () {
		this.group.remove();
	};
}

function syncEffects(critter, bundle) {
	// 1. Clear existing visual icons for this critter
	if (critter.getEffecttts()) {
		while (critter.getEffecttts().length > 0) {
			var effect = critter.getEffecttts().pop();
			effect.getImg().remove();
			// If your Effect object has text/stacks, remove those too:
			if (effect.stackText) effect.stackText.remove();
		}
	}

	if (bundle === "none" || !bundle) return;

	// 2. Parse the bundle (Split by ; for each effect)
	var effectDataStrings = bundle.split(";");

	for (var i = 0; i < effectDataStrings.length; i++) {
		if (effectDataStrings[i] === "") continue;

		// Split by : (img:duration:name:description:stacks)
		var parts = effectDataStrings[i].split("^");
		var imgName = parts[0];
		var type = parts[1];
		var effect = parts[2];
		var source = parts[3];
		var duration = parts[4];
		var stacks = parseInt(parts[5]);

		// 3. Calculate Position (Using your existing logic)
		var portraitX = critter.getPortraitX();
		var portraitY = critter.getPortraitY();
		var xOffset = 15 * i; // Space icons apart
		var x;

		if (critter.getSide() === "right") {
			x = portraitX - 54 - xOffset;
		} else {
			x = portraitX + 42 + xOffset;
		}
		// 4. Create and store the new effect
		var newEffect = new Effect(x, portraitY - 5, imgName, type, effect, source, duration, stacks);
		critter.getEffecttts().push(newEffect);

		// 6. Setup Hover/Tooltip
		setupEffectHover(newEffect);
	}
}

function setupEffectHover(effect) {
	// 1. We use .group because it contains both the image and the stack text
	// This ensures the hover doesn't "flicker" when moving between icon and number
	var hoverArea = effect.group;
	...
	hoverArea.hover(
		function () {
			...
			// Send the raw description to the InfoBox
			// The InfoBox.setInfo method we updated will handle the wrapping
			infoBox.setInfo(effect.getDescription());
		},
		function () {
			// Clear the info box when the mouse leaves
			infoBox.setInfo("");
...
function wrapText(text, limit) {
	...
	words.forEach(function (word) {
		// Handle existing newlines in the data
		if (word.includes("\n")) {
			var parts = word.split("\n");
			parts.forEach(function (part, index) {
				if ((currentLine + part).length > limit) {
...
function triggerActionAnimation(type, targetType, target, side, subSide) {
	console.log("triggerActionAnimation: " + type + ', ' + targetType + ', ' + target + ', ' + side);
	var ANIMATION_SPEED_MS = 120;
	var FRAME_WIDTH = 200;
	var FRAME_HEIGHT = 200;
	var TOTAL_FRAMES = 4;
	...
	var DISPLAY_X = (targetType === "square" || targetType === "spot")
		? target.getCritterX()
		: target.getImage().attr('x');
	var DISPLAY_Y = (targetType === "square" || targetType === "spot")
		? target.getCritterY()
		: target.getImage().attr('y');

	var flashImage = gridPaper.image("assets/" + spritesheetFileName, 0, 0, FRAME_WIDTH * TOTAL_FRAMES, FRAME_HEIGHT)
		.attr({
			opacity: 1,
			x: DISPLAY_X,
			y: DISPLAY_Y,
	...
	if (side === "right" && type === "BLOCK" || subSide === "left" && type === "ATTACK") {
		var centerX = DISPLAY_X + FRAME_WIDTH / 2;
		var centerY = DISPLAY_Y + FRAME_HEIGHT / 2;
		flashImage.transform("S-1,1," + centerX + "," + centerY);
	}
	...
	var animationInterval = setInterval(function () {
		...
		if (currentFrame >= TOTAL_FRAMES) {
			clearInterval(animationInterval);
			flashImage.animate({ opacity: 0 }, 100, function () {
				this.remove(); // Completely removes from DOM/Raphael
			});
			return;
		}

		var CLIP_OFFSET_X = currentFrame * FRAME_WIDTH;
		flashImage.attr({ x: DISPLAY_X - CLIP_OFFSET_X });

		// Re-apply transform after moving X to keep it flipped
		if ((side === "right" && type === "BLOCK") || (subSide === "left" && type === "ATTACK")) {
			var centerX = DISPLAY_X + FRAME_WIDTH / 2;
			var centerY = DISPLAY_Y + FRAME_HEIGHT / 2;
			flashImage.transform("S-1,1," + centerX + "," + centerY);
...
function animate(str) {
	...
	updateActionLog("action," + "~~~~~~~~ROUND " + roundCount + "~~~~~~~~");
	if (!str.startsWith("ANIMMANIFEST")) return;
	calculateTurn("calculating,begin");
	roundCount += 1;
	if (timeout) clearTimeout(timeout);
	timeout = setTimeout(function () {
		calculateTurn("calculating,end");
	}, 10000);

	var pieces = str.split("|").slice(1);
	...
	pieces.forEach(function (entry) {
		...
		// UPDATED JS MAPPING (Requires matching Java String.format)
		var behaviour = t[1];
		var abilityName = t[2];
		var abilityType = t[3];
		var subName = t[4];
		var subSide = t[5];
		var subCurHP = parseInt(t[6]);  // NEW
		var subMaxHP = parseInt(t[7]);  // NEW
		var subCurE = parseInt(t[8]);  // SHIFTED
		var subMaxE = parseInt(t[9]);  // SHIFTED
		var tarType = t[10];           // SHIFTED
		var tarName = t[11];           // SHIFTED
		var tarSide = t[12];           // SHIFTED
		var tarCurHP = parseInt(t[13]); // SHIFTED
		var tarMaxHP = parseInt(t[14]); // SHIFTED
		var delay = parseFloat(t[15]) * 1000;
		var logStr = t[16];
		var subBundle = t[17];
		var tarBundle = t[18];
		var sub = identifyCritter(subName, subSide);
		console.log(abilityName + ", " + tarName + ", " + logStr);

		setTimeout(function () {
			console.log("behaviour" + behaviour);

			if (behaviour === "GameOver") {
				if (bar) {
					bar.stop();
				}
				...
				var winner = "draw";
				if (logStr.includes("P1")) winner = "P1";
				else if (logStr.includes("P2")) winner = "P2";

				var result = "draw";
				if (winner !== "draw") {
					var winnerSide = (winner === "P1") ? "left" : "right";
					result = (SIDE === winnerSide) ? "win" : "loss";
				}

				gameOver("gameover," + result);
				return; // Stops execution of THIS specific timeout
			}

			else if (behaviour === "Tick") {
				if (logStr && logStr !== "none") {
					var messages = logStr.split(" ;; ");
					var effects = subBundle.split(";");

					// 1. Determine the prefix based on subSide
					var playerPrefix = (subSide === "left") ? "P1" : "P2";

					// 2. Format the Header: "P1 Lion ticks:"
					updateActionLog("tick,<b>" + playerPrefix + " " + subName + " ticks:</b>");

					messages.forEach(function (msg, index) {
						var abilityName = "Effect";

						if (effects[index]) {
							var effectParts = effects[index].split("^");
							if (effectParts.length > 3) {
								abilityName = effectParts[3];
							}
						}

						// 3. List the indented e ffect with the ability name at the end
						updateActionLog("tick,&nbsp;&nbsp; " + msg.trim() + " [" + abilityName + "]");
					});
				}
			}

			else if (behaviour === "STEALTH") {
				var critter = identifyCritter(tarName, tarSide);
				timeout = setTimeout(function () {
					critter.getImages().attr('opacity', 0.3);
					critter.getImage().attr('opacity', 0.3);
				}, 200);

			} else if (behaviour === "UNSTEALTH") {
				console.log("unstealth");
				var critter = identifyCritter(tarName, tarSide);
				timeout = setTimeout(function () {
			...
			console.log("in timeout: " + abilityName + ", " + tarName);

			if (logStr && logStr !== "none" && behaviour !== "Tick") {
				// Format it nicely. If it's a standard action, maybe add a prefix
				updateActionLog("action," + logStr);
			}

			var isFailure = logStr.toLowerCase().includes("failed");

			if (!isFailure) {

				// 1. Determine the visual target
				var targetVisualObj = null;
				if (tarType === "critter") {
					targetVisualObj = identifyCritter(tarName, tarSide);
					console.log("tarCurHP: " + tarCurHP);
					targetVisualObj.setHealth(tarCurHP);
					syncEffects(targetVisualObj, tarBundle);
					// changeBars(tarName, tarSide, "energy", tarCurE);
				} else if (tarType === "square") {
					targetVisualObj = identifySpot(tarName);
				}
				console.log("ability name: " + abilityName);
				// 2. Trigger the Sprite Animation
				if (abilityName == "MOVE") {
					console.log("MOVE," + abilityName + "," + subName + "," + subSide + "," + tarName);
					move(abilityName + "," + subName + "," + subSide + "," + tarName)
				} else if (targetVisualObj && behaviour === "basic") {
					var critter = identifyCritter(subName, subSide);
					...
					triggerActionAnimation(abilityType, tarType, targetVisualObj, tarSide, subSide);
				}

			} else {
				console.log("Action failed - skipping visual animations.");
				// Optional: Play a "fizzle" sound or show a small "X" over the subject
			}
			sub.setEnergy(subCurE);
			sub.setHealth(subCurHP);

			syncEffects(sub, subBundle);
...
// function changeBars(fighterName, fighterSide, barType, value) {
//     var critter = identifyCritter(fighterName, fighterSide);
//     if (barType == "health") {
//         critter.setHealth(value);
//     } else if (barType == "energy") {
...
function displayActionQueue(str) {
	console.log("displayActionQueue: " + str + ", calculating: " + calculating);
	if (!calculating) {
			...
			if (!initialPhase && !locked) {
				...
				if (ly + initialY >= 316 && ly + initialY <= 444) {
					...
					if (midX <= 130) {
						...
						lineX = newX;
						if (item.isQueued()) {
							ws.send("click,item,remove," + item.getName() + "," + item.getPos() + "," + 0 + "," + id);
						}
						ws.send("click,item,add," + item.getName() + "," + item.getPos() + "," + 0 + "," + id);
						...
						lineX = newX;
						if (item.isQueued()) {
							ws.send("click,item,remove," + item.getName() + "," + item.getPos() + "," + 10 + "," + id);
						}
						ws.send("click,item,add," + item.getName() + "," + item.getPos() + "," + 10 + "," + id);
					} else {
						...
						for (var i = 0; i < 11; i++) {
							if (midX <= leftX) {
								...
								if (leftX - midX <= midX - (leftX - 0.1 * (WIDTH - 260))) {
									newX = leftX;
									if (item.isQueued()) {
										ws.send("click,item,remove," + item.getName() + "," + item.getPos() + "," + pos + "," + id);
									}
									ws.send("click,item,add," + item.getName() + "," + item.getPos() + "," + (i) + "," + id);
									...
									ws.send("click,item,add," + item.getName() + "," + item.getPos() + "," + (i - 1) + "," + id);
									pos = i - 1;
								}
								var newNewX = newX;
...
Raphael.st.iconDraggable = function (icon, arr) {
	...
	moveFnc = function (dx, dy) {
		if (!locked) {
			...
			// --- 1. THE LIFT (Your Original Perfect Logic) ---
			if ((ly < -70 || ly > 70) && !collapsed) {
				var iconPos = arr.indexOf(icon);
				arr.splice(iconPos, 1);
				collapsed = true;

				// Re-stack the bar icons while you are in the air
				for (var i = 0; i < arr.length; i++) {
					if (i == 0) {
						arr[i].setX(130);
					} else {
						arr[i].setX(arr[i - 1].getX2());
					}
					slide(arr[i].getX(), arr[i]);
				}
			}

			// --- 2. THE RE-ENTRY (Your Original Insertion Logic) ---
				...
				if (arr.length == 0) {
					...
					// Find where to drop it back in
					for (var i = 0; i < arr.length; i++) {
						var prevIconMidPoint = arr[i].getX() + arr[i].getWidth() / 2;
						var nextIconMidPoint = (arr[i + 1]) ? (arr[i + 1].getX() + arr[i + 1].getWidth() / 2) : 999999;
				...
				// Update physical positions after re-entry
				for (var i = 0; i < arr.length; i++) {
					if (i == 0) {
						arr[i].setX(130);
					} else {
			...
			// --- 3. THE GROUNDED SWAP (New 1px Logic) ---
			else if (!collapsed) {
				var iconMidPoint = origIconX + lx + icon.getWidth() / 2;
				var currentIdx = arr.indexOf(icon);
				var swapped = false;

				// Check Right
				if (currentIdx < arr.length - 1) {
					var nextMid = arr[currentIdx + 1].getX() + arr[currentIdx + 1].getWidth() / 2;
					if (iconMidPoint > nextMid) {
						arr.splice(currentIdx, 1);
						arr.splice(currentIdx + 1, 0, icon);
						newIconPos = currentIdx + 1;
						swapped = true;
					}
				}
				// Check Left
				if (!swapped && currentIdx > 0) {
					var prevMid = arr[currentIdx - 1].getX() + arr[currentIdx - 1].getWidth() / 2;
					if (iconMidPoint < prevMid) {
						arr.splice(currentIdx, 1);
						arr.splice(currentIdx - 1, 0, icon);
						newIconPos = currentIdx - 1;
						swapped = true;
				...
				// If a swap happened, slide the neighbors to their new spots
				if (swapped) {
					for (var i = 0; i < arr.length; i++) {
						if (i == 0) {
							arr[i].setX(130);
			...
			// Always update the visual drag position
			me.transform('t' + lx + ',' + ly);
			var rect = icon.getIcon()[0];
			icon.getIcon()[1].attr({
				"clip-rect": (rect.attr('x') + " " + rect.attr('y') + " " + rect.attr('width') + " " + rect.attr('height'))
			});
		}
	}

	// HELPER: To keep moveFnc clean, move your sliding loop here
	...
	startFnc = function () {
		...
		endFnc = function () {
			if (!locked) {
				...
				// Debugging Log 1: State at release
				console.log("DRAG END - origPos:", origPos, " | newIconPos:", newIconPos, " | id:", id);

				if (ly < -70 || ly > 70) {
					console.log("ACTION: Deleting Icon");
					critters.forEach(function (f) { f.unindicateAttack(); f.unindicateSupport(); f.unindicateBlock(); });
					spots.forEach(function (s) { s.unindicateMove(); s.unindicateAttack(); s.unindicateSupport(); });

					if (ws && ws.readyState === 1) {
						ws.send("click,actionqueue,remove," + origPos + "," + origPos + "," + id);
					}
					icon.remove();
				} else {
					...
					// Debugging Log 2: Check the "If" condition
					if (origPos !== newIconPos) {
						console.log("ACTION: Sending Switch to Server:", origPos, "to", newIconPos);
						if (ws && ws.readyState === 1) {
							ws.send("click,actionqueue,switch," + origPos + "," + newIconPos + "," + id);
						} else {
							console.error("WS ERROR: Cannot send switch. ReadyState:", ws.readyState);
						}
						// Sync the position for the next move
						origPos = newIconPos;
					} else {
						console.warn("NO ACTION SENT: origPos and newIconPos are the same.");
					}
...
readyButton.getRect().click(function () {
	...
	if (initialPhase) {
		...
		if (formationReadied) {
			...
			ws.send("click,initialposition,unready," + id);
			locked = false;
			return;
		}
		if (quickStartEnabled) {
...
function icon(time, energy, user, target, description, x, type, targetSide, indicatorMessage, info, y) {
	...
	// For the User Name (e.g., PLAYER_ONE -> Player one)
	this.nameText = gridPaper.text(this.x + this.width / 2, this.y + 12, formatText(this.user)).attr({
		"font-family": iconFont,
		"font-size": "12"
	});

	// For the Description (e.g., SMOKE_BOMB -> Smoke bomb)
	this.descText = gridPaper.text(this.x + this.width / 2, this.y + this.height / 2, formatText(this.description)).attr({
...
function Ability(clickable, time, energy, name, description, pos, damage, type, effects) {
	...
	this.nameText = gridPaper.text(this.x + this.width / 2, this.y + 10, this.name
		.split('_')
...
function Critter(name, maxHealth, maxEnergy, side, pos, passiveName, passiveDescription) {
	...
	this.setHealth = function (health) {
		if (this.currentHealth - health != 0) {
			...
			var dHS = dHealth > 0 ? "+" + dHealth : dHealth;
			var color = dHealth > 0 ? '#00A053' : '#E63B00';

			var newText = gridPaper.text(this.x + 100, this.y + 100 + 15 * this.damageText.length, dHS)
				.attr({ 'font-size': 25, 'fill': color, 'font-family': dTextFont });

			var self = this; // Capture 'this' for the callback
			newText.animate({ y: this.y + 100 - 40 + 15 * this.damageText.length, 'opacity': 0 }, 2500, function () {
				newText.remove();
				// Better to filter the specific object out of the array
				var index = self.damageText.indexOf(newText);
				if (index > -1) self.damageText.splice(index, 1);
			});
		...
		if (health <= 0) {
			// Use 'this.death()' instead of 'death()'
			this.death();
		} else if (this.currentHealth <= 0 && health > 0) {
			this.revive();
		}
		...
		// Health bar logic
		var barWidth = this.currentHealth / maxHealth * 80;
		if (this.portraitX < 700) {
			this.healthBar.animate({ width: barWidth }, 8);
		} else {
			this.healthBar.animate({ width: barWidth, x: this.portraitX - 31 - barWidth }, 8);
		}

		this.healthText.attr('text', this.currentHealth + "/" + maxHealth);
		this.healthText.node.setAttribute("class", "donthighlight");
	...
	this.clearEffects = function () {
		var origLength = this.getEffecttts().length;
		for (var i = 0; i < origLength; i++) {
			createEffectTT("effecttt,remove," + tokens[1] + "," + tokens[2] + "," + 0);
		}
	}
...
function move(moves) {
	//		alert(moves);
	console.log("FUNCTION MOVE: " + moves)
